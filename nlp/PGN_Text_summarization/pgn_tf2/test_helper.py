...
def batch_greedy_decode(model, batch, vocab, params):
    encoder_batch_data=batch[0]
    # 判断输入长度
    batch_size = params["batch_size"]

    enc_output, enc_hidden = model.call_encoder(encoder_batch_data["enc_input"])
    # 2. 复制
    dec_hidden = enc_hidden
    # 3. <START> * BATCH_SIZE
    dec_input = tf.expand_dims([vocab.word_to_id(vocab.START_DECODING)] * batch_size, 1)

    preidicts, _, attentions, coverages = model.callTest(dec_input,
                                                  dec_hidden,
                                                  enc_output,
                                                  params['max_dec_len'],
